...
import com.coinzway.coinz4s.btc.ClientObjects.{AddressType, RawTransactionInput, RawTransactionInputs}
import com.coinzway.coinz4s.btc.Responses.{CoinbaseInput, TransactionInput}
...
class BitcoinClientTest extends AnyFlatSpec with Matchers with TestDataHelper {
  ...
  val bitcoinClient: BitcoinClient[Id] = BitcoinClient(user, password, host, port)
