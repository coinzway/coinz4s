...
class LitecoindClientTest extends AnyFlatSpec with Matchers with TestDataHelper {
  val user = "user"
  ...
  it should "new address should handle parse error" in {
    litecoindClient.getNewAddress(Some("parseError"), None) match {
      case Left(x) =>
  ...
  it should "handle insufficient funds errors" in {
    litecoindClient.sendToAddress("nt54hMq9ghkvTBqmw3BoLjPBGBPWU1RexJ", 101) match {
      case Left(x) =>
        x shouldBe a[GeneralErrorResponse]
  ...
  it should "respond with error for out of range tx fee" in {
    litecoindClient.setTxFee(BigDecimal(-1)) match {
      case Left(err) =>
  ...
  it should "get raw transaction by id" in {
    ...
    litecoindClient.getRawTransactionVerbose(txid) match {
      ...
      case Right(response) =>
        ...
        txId shouldBe "aa17244e56d4dd1e0548901ba27f85291dec508dc8c79b47ba16879fd601a576"
    }
  }
  ...
  "signrawtransaction" should "return signed transaction" in {
    ...
    litecoindClient.signRawTransactionWithWallet(hex) match {
      case Left(_) => throw new RuntimeException("unexpected litecoind response")
  ...
  "any endpoint" should "return error for error response" in {
    val response = litecoindClient.validateAddress("invalid-json")
